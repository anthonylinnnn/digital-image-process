...
void histogram_equalization(unsigned char *image_data, int width, int height) {
    ...
    float cumulative_distribution[256] = {0.0};
    ...
    // 計算累積分佈函數
    cumulative_distribution[0] = (float)histogram[0] / total_pixels;
    for (int i = 1; i < 256; i++) {
        cumulative_distribution[i] = cumulative_distribution[i - 1] + (float)histogram[i] / total_pixels;
        if (histogram[i] != 0) {
...
int main(int argc, char *argv[]) {
    ...
    save_bmp("boat_new5.bmp", R, R, R); // for gray images
