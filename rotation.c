...
void rotation(int src[MaxBMPSizeX][MaxBMPSizeY], double ang, int width, int height, int dst[1024][1024]) {
    ...
    int x_h,y_h,new_x_h,new_y_h;//center of bmp
    int x, y,new_x, new_y;

    int N_border_width=abs((int)((cos_angle * width ) + (sin_angle * height)));
    int N_border_height=abs((int)((sin_angle * width ) + (cos_angle * height)));
...
int main(){
    ...
    if (color_type=0){
        ...
        printf("gray image\n");
        save_bmp("rotation.bmp",RP,RP,RP);
    }
