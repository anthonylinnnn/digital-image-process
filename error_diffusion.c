...
void color_phase_adjust(int Vi[1024][1024],int width,int height,int th,int Vo){
    int x,y;
    int Cnt=1;
    double Vs=0;
    for(y=0;y<height;y++){
        for(x=0;x<width;x++){Vs+=(Vi[x][y]-th);Cnt++;}
    }   
    Vo=(int)(Vs/Cnt);
    printf("offset = %d\n",Vo);
}

void error_diffusion_improved2(unsigned char *image_data, int width, int height,int threshold) {
    int x, y;
    ...
    color_phase_adjust(RR,width,height,threshold,offset_r);
    color_phase_adjust(GR,width,height,threshold,offset_g);
    color_phase_adjust(BR,width,height,threshold,offset_b);
    printf("offset_r = %d\n",offset_r);
    printf("offset_g = %d\n",offset_g);
    printf("offset_b = %d\n",offset_b);
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            old_pixel_b = image_data[index + 0];
            ...
            image_data[index + 0] = new_pixel_b;
            ...
            if (x + 1 < width) {
                image_data[index + 3 + 2] += quant_error_r * 8 / 16;
                image_data[index + 3 + 1] += quant_error_g * 7 / 16;
                image_data[index + 3 + 0] += quant_error_b * 7 / 16;
            }
            if (x - 1 >= 0 && y + 1 < height) {
                image_data[index + width * 3 - 3 + 2] += quant_error_r * 1 / 16;
                image_data[index + width * 3 - 3 + 1] += quant_error_g * 2 / 16;
                image_data[index + width * 3 - 3 + 0] += quant_error_b * 2 / 16;
            }
            if (y + 1 < height) {
                image_data[index + width * 3 + 2] += quant_error_r * 5 / 16;
                image_data[index + width * 3 + 1] += quant_error_g * 5 / 16;
                image_data[index + width * 3 + 0] += quant_error_b * 5 / 16;
            }
            if (x + 1 < width && y + 1 < height) {
                image_data[index + width * 3 + 3 + 2] += quant_error_r * 2 / 16;
                image_data[index + width * 3 + 3 + 1] += quant_error_g * 2 / 16;
                image_data[index + width * 3 + 3 + 0] += quant_error_b * 2 / 16;
            }
        }
...
void error_diffusion_improved(unsigned char *image_data, int width, int height, int threshold) {
    int x, y;
    int index;
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            for (int color = 0; color < 3; color++) {
                old_pixel = image_data[index + color];
                new_pixel = (old_pixel >= threshold) ? 255 : 0;
                image_data[index + color] = new_pixel;
                sum_old += old_pixel;
                sum_new += new_pixel;
            }

            quant_error = (sum_old - sum_new) / 3;
            if (x + 1 < width) {
                for (int color = 0; color < 3; color++) {
                    image_data[index + 3 + color] += quant_error * 7 / 16;
                }
            }
            if (y + 1 < height) {
                if (x > 0) {
                    for (int color = 0; color < 3; color++) {
                        image_data[index + width * 3 - 3 + color] += quant_error * 3 / 16;
                    }
                }
                for (int color = 0; color < 3; color++) {
                    image_data[index + width * 3 + color] += quant_error * 5 / 16;
                }

                if (x + 1 < width) {
                    for (int color = 0; color < 3; color++) {
                        image_data[index + width * 3 + 3 + color] += quant_error * 1 / 16;
                    }
                }
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    printf("adjust threshold gate (default = 127) = \n");   
    scanf("%d",&threshold); 
    printf("\n=====\nthreshold gate = %d\n",threshold);   

    // open_bmp("lena_color_512.bmp", R, G, B, width, height); // for true color images
    open_bmp("lena_std.bmp", RR, GR, BR, &width, &height); 

    error_diffusion_improved(bmp_tmp.image_data, width, height, threshold);
    for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
            int index = y * width * 3 + x * 3;
            RP[x][y] = bmp_tmp.image_data[index + 2];
            GP[x][y] = bmp_tmp.image_data[index + 1];
            BP[x][y] = bmp_tmp.image_data[index + 0];
        }
    }
    //save_bmp("lena_std_new5.bmp", R, R, R); // for gray images
    save_bmp("lena_std_RGB.bmp", RP, GP, BP);
    printf("Job Finished!\n");
    close_bmp();
